...
public class OpenRouterAiService {
    ...
    // Каскад бесплатных моделей. OpenRouter принимает массив "models"
    // (МАКСИМУМ 3 элемента — больше нельзя, иначе 400) и сам переключается
    // на следующую, если предыдущая ответила 402/429/5xx. Llama 3.3 70B —
    // приоритетная: лучшая связность на русском. Qwen 3 Next и GLM 4.5 Air —
    // запасные с реально работающим free-tier у других провайдеров.
    ...
    public ScoreExplainResponse explainScore(ScoredPropertyDto scored, SearchProfile profile) {
        ...
        try {
            ...
            if (content.isBlank()) {
                log.warn("[AI] OpenRouter вернул пустой ответ (model={}). Raw: {}", usedModel, response);
                return fallback();
            }
            log.info("[AI] Объяснение сформировано ({} символов, model={})", content.length(), usedModel);
    ...
    private String buildUserPrompt(ScoredPropertyDto scored, SearchProfile profile) {
        ...
        return """
                Ты пишешь на русском языке развёрнутый блочный разбор оценки коммерческого помещения. Тебе даны ФАКТЫ — пересказывай только их. ИНЫХ ФАКТОВ НЕТ.

                ЖЁСТКАЯ СТРУКТУРА ОТВЕТА — РОВНО ЭТИ 6 БЛОКОВ, В ЭТОМ ПОРЯДКЕ:

                ФИНАНСЫ
                <1–2 предложения: площадь, цена, как соотносятся с бюджетом и диапазоном площади. Конкретные цифры из ФАКТОВ. Балл X/20.>

                ТЕХНИКА
                <1–2 предложения: ключевые штрафы (что отсутствует или не указано, на сколько баллов). Если штрафов нет — так и напиши. Балл X/20.>

                КОНКУРЕНТЫ
                <2–3 предложения: сколько прямых конкурентов в радиусе, КОНКРЕТНЫЕ имена по правилу ниже, упомяни безымянных если их N>0. Балл X/40.>

                СИНЕРГИЯ
                <1–2 предложения: сколько желаемых категорий задано, сколько найдено бизнесов рядом, 2–3 имени для иллюстрации. Если соседи не заданы — так и пиши. Балл X/15.>

                ТРАНСПОРТ
                <1 предложение: тип ближайшей остановки, её расстояние, балл. Балл X/5.>

                ИТОГ
                <1 предложение: «N/100, главное ограничение — …» либо «N/100, ограничений по данным алгоритма нет». Без советов и пожеланий.>

                ЖЁСТКИЕ ПРАВИЛА:
                - Заголовки блоков — РОВНО как выше (ЗАГЛАВНЫМИ, на отдельной строке, без двоеточий, без emoji, без markdown-звёздочек).
                - Между блоками — ОДНА пустая строка. Внутри блока — сплошной текст без переносов.
                - Только русский, без вводных слов («Конечно», «Рассмотрим»), без расплывчатых формулировок («требует улучшений», «относительно неплохо»).
                - Запрещено объяснять алгоритм (что «40 это максимум» и т.д.). Только цифры из ФАКТОВ.
                - Конкурентов упоминай ПОИМЁННО:
                    • если прямых ≤ 5 — назови КАЖДОГО;
                    • 6–15 — назови первые 5 из списка и допиши «и ещё N»;
                    • ≥ 16 — назови первые 3 наиболее узнаваемых и допиши «и ещё N».
                - Имена бери из списка в ФАКТАХ — не выдумывай и не заменяй на «несколько аптек». Если в ФАКТАХ есть «безымянных N» — добавь это число во фразу.
                - Не выходи за 6 блоков, не добавляй своих разделов («Рекомендации», «Заключение» и т.п.).

                ПРИМЕР (для других данных):
                ФИНАНСЫ
                Помещение 65 м² за 320 000 ₽/мес попадает в бюджет 250 000–400 000 ₽ и в диапазон площади 50–80 м². Балл 20/20.

                ТЕХНИКА
                Вода, вентиляция, отдельный вход и санузел есть; парковка не указана, штраф 1.0. Балл 19/20.

                КОНКУРЕНТЫ
                В радиусе 1 км найдено 22 прямых конкурента: «36,6», «Ригла», «Самсон-фарма», «Горздрав», «Неофарм» и ещё 17, плюс 4 без названия. Высокая плотность ближайших аптек роняет балл. Балл 0/40.

                СИНЕРГИЯ
                Желаемые соседи не заданы, поэтому балл идёт по умолчанию. Балл 15/15.

                ТРАНСПОРТ
                Метро «Арбатская» в 220 м даёт почти максимум. Балл 4/5.

                ИТОГ
                58/100, главное ограничение — высокая плотность прямых конкурентов.

                ФАКТЫ:
                %s

                Теперь напиши разбор по этим ФАКТАМ в указанном формате, начни сразу с блока ФИНАНСЫ.
                """.formatted(facts);
    }
    ...
    private String buildCompetitorBreakdown(ScoredPropertyDto scored) {
        ...
        if (direct.isEmpty()) {
            sb.append("- в радиусе поиска не найдено прямых конкурентов; балл максимальный, 40/40.\n");
            return sb.toString();
        }

        // Имена в списке уже отсортированы по близости (ближайший первым) —
        ...
        sb.append("- логика балла: distance-weighted exp-decay. Близкий конкурент весит ~1, ")
          .append("дальний в радиусе 1 км — ~0.05. Score = 40 · exp(-1.0·Σw_прямых). ")
    ...
    private String buildRequestBody(String userPrompt) {
        try {
            ...
            // "model" — основная (первая в каскаде), "models" — список fallback'ов.
            // OpenRouter переключится на следующую модель из массива при 402/429/5xx.
            root.put("model", MODEL_FALLBACK_CHAIN.get(0));
            ArrayNode models = root.putArray("models");
            for (String m : MODEL_FALLBACK_CHAIN) models.add(m);
